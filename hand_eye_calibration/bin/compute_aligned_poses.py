#!/usr/bin/env python
from hand_eye_calibration.time_alignment import (calculate_time_offset_angV,
...
from hand_eye_calibration.csv_io import (
    ...
    read_time_stamped_angular_velocity_from_csv_file,
    read_time_stamped_poses_from_txt_file,
    read_time_stamped_angular_velocity_from_txt_file,
)
...
if __name__ == '__main__':
    ...
    if not args.txt:
        ...
        if not args.imu:
            ...
            (times_W_E, angV_W_E) = read_time_stamped_angular_velocity_from_csv_file(
        ...
        (time_stamped_poses_B_H, times_B_H,
         quaternions_B_H) = read_time_stamped_poses_from_txt_file(
            args.poses_B_H_csv_file, use_JPL_quaternion)
        print("Found ", time_stamped_poses_B_H.shape[
            0], " poses in file: ", args.poses_B_H_csv_file)
        ...
        if not args.imu:
            (time_stamped_poses_W_E, times_W_E,
             quaternions_W_E) = read_time_stamped_poses_from_txt_file(
                args.poses_W_E_csv_file, use_JPL_quaternion)
            print("Found ", time_stamped_poses_W_E.shape[
                0], " poses in file: ", args.poses_W_E_csv_file)
        else:
            # read imu angular velocity
            (times_W_E, angV_W_E) = read_time_stamped_angular_velocity_from_txt_file(
                args.poses_W_E_csv_file)
            # print("Found ", time_stamped_poses_W_E.shape[
            #     0], " poses in file: ", args.poses_W_E_csv_file)
    ...
    if  not args.imu:
      ...
      time_offset = calculate_time_offset_angV(times_B_H, quaternions_B_H, times_W_E,
                                    angV_W_E, filtering_config,
                                    filtering_config.visualize)
